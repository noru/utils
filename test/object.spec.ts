...
describe('ObjectUtil', () => {
  it('shallowEqual', () => {
    expect(ObjectUtil.shallowEquals([], [])).to.be.true;
    expect(ObjectUtil.shallowEquals([1, 2, 3], [1, 2, 3])).to.be.true;

    expect(ObjectUtil.shallowEquals([1], [])).to.be.false;
    expect(ObjectUtil.shallowEquals([{}], [{}])).to.be.false;

    expect(ObjectUtil.shallowEquals({}, {})).to.be.true;
    expect(ObjectUtil.shallowEquals({ a: 1 }, { a: 1 })).to.be.true;
    expect(ObjectUtil.shallowEquals({ a: 1 }, { a: 2 })).to.be.false;
    expect(ObjectUtil.shallowEquals({ a: 1, b: {} }, { a: 1, b: {} })).to.be.false;
  });
  ...
  it('flattenDeepBy', () => {
    ...
    let secLvl = [
      {
        ...
        name: '王大锤',
        timestamp: new Date(),
        content: 'Defend this one, conservatives.',
        subComments: thirdLvl,
      },
      {
        id: '4',
        name: '王小锤',
        timestamp: new Date(),
        content: ' Why are you discussing tariffs at the soup store!?',
      },
    ];
    ...
    let topLvl = [
      // tslint:disable-next-line:max-line-length
      {
        ...
        timestamp: new Date(),
        content:
          "Y'know, we really have the best Stupid. It's American, American Stupid, and it's so great. It's not like that European Stupid, I've seen their stupid... y'know, I've been all over the world, I've met a lot of very rich",
        subComments: secLvl,
      },
